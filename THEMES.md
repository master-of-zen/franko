# Franko Theme Collection

A comprehensive list of popular themes to be implemented in Franko reader.

## Implementation Status

- [x] = Implemented
- [ ] = Planned

---

## Dark Themes

### Editor/IDE Inspired

- [x] Tokyo Night
- [x] Dracula
- [x] Nord
- [x] One Dark Pro
- [x] Atom One Dark
- [x] Material Dark
- [x] Palenight
- [x] Night Owl
- [x] Shades of Purple
- [x] Synthwave '84
- [ ] Cyberpunk
- [x] Ayu Dark
- [x] Ayu Mirage
- [ ] Moonlight
- [ ] Moonlight II
- [ ] Vitesse Dark
- [x] GitHub Dark
- [ ] GitHub Dark Dimmed
- [ ] GitHub Dark High Contrast
- [x] Rosé Pine
- [x] Rosé Pine Moon
- [x] Everforest Dark
- [x] Kanagawa
- [ ] Kanagawa Wave
- [ ] Kanagawa Dragon
- [x] Poimandres
- [x] Vesper
- [ ] Mellow
- [x] Oxocarbon Dark
- [x] Flexoki Dark
- [ ] Bluloco Dark
- [ ] Bearded Theme Arc
- [ ] Bearded Theme Black & Gold
- [ ] Bearded Theme Coffee
- [ ] Houston
- [ ] Min Dark
- [ ] Noir
- [ ] Pitch Black

### Solarized Family

- [x] Solarized Dark
- [ ] Solarized Dark Higher Contrast

### Gruvbox Family

- [x] Gruvbox Dark
- [ ] Gruvbox Dark Hard
- [ ] Gruvbox Dark Soft
- [ ] Gruvbox Material Dark
- [ ] Gruvbox Material Dark Hard
- [ ] Gruvbox Material Dark Soft

### Catppuccin Family

- [x] Catppuccin Mocha
- [x] Catppuccin Macchiato
- [x] Catppuccin Frappé

### Monokai Family

- [x] Monokai
- [ ] Monokai Pro
- [ ] Monokai Pro Filter Machine
- [ ] Monokai Pro Filter Octagon
- [ ] Monokai Pro Filter Ristretto
- [ ] Monokai Pro Filter Spectrum
- [ ] Monokai Classic
- [ ] Monokai Dimmed

### Terminal Inspired

- [ ] Hyper
- [ ] Snazzy
- [x] Cobalt2
- [x] Horizon
- [x] Iceberg
- [ ] Jellybeans
- [ ] Tender
- [ ] Wombat
- [x] Zenburn
- [ ] Base16 Ocean Dark
- [ ] Base16 Tomorrow Night
- [ ] Base16 Eighties

### Retro/Gaming

- [ ] Noctis
- [ ] Noctis Azureus
- [ ] Noctis Bordo
- [ ] Noctis Minimus
- [ ] Laserwave
- [ ] SynthWave '84
- [ ] Outrun
- [ ] Hotline
- [ ] Hopscotch
- [ ] Spacegray
- [ ] Spacegray Eighties
- [ ] Fairyfloss
- [ ] Commodore 64

### Minimalist Dark

- [ ] Subliminal
- [ ] Quiet Light Dark
- [ ] One Monokai
- [ ] Plastic
- [ ] Rainglow (various)
- [ ] Andromeda
- [ ] Lucy
- [ ] Palenight Italic
- [ ] Remedy Dark
- [ ] Dark+
- [ ] Visual Studio Dark

---

## Light Themes

### Classic Light

- [x] Light (Default)
- [ ] Quiet Light
- [x] Solarized Light
- [x] GitHub Light
- [ ] GitHub Light Default
- [ ] GitHub Light High Contrast
- [x] Atom One Light
- [ ] One Light
- [ ] Xcode Light
- [ ] IntelliJ Light
- [ ] Visual Studio Light
- [ ] Winter is Coming Light
- [ ] Bluloco Light
- [ ] Min Light

### Gruvbox Light Family

- [x] Gruvbox Light
- [ ] Gruvbox Light Hard
- [ ] Gruvbox Light Soft
- [ ] Gruvbox Material Light
- [ ] Gruvbox Material Light Hard
- [ ] Gruvbox Material Light Soft

### Catppuccin Light

- [x] Catppuccin Latte

### Nature/Organic Light

- [x] Everforest Light
- [x] Rosé Pine Dawn
- [x] Flexoki Light
- [x] Ayu Light
- [ ] Vitesse Light

### Paper/Reading Optimized

- [x] Sepia
- [x] Paper
- [ ] Ivory
- [ ] Parchment
- [ ] Cream
- [ ] Newspaper
- [ ] E-Ink
- [x] Kindle
- [ ] iBooks Light
- [x] Kobo Light
- [ ] Nook Light

---

## High Contrast Themes

- [x] High Contrast (Dark)
- [ ] High Contrast Light
- [ ] WCAG AAA Dark
- [ ] WCAG AAA Light
- [ ] Dyslexia Friendly Dark
- [ ] Dyslexia Friendly Light
- [ ] Low Vision Dark
- [ ] Low Vision Light

---

## Reading-Specific Themes

### E-Reader Inspired

- [ ] Kindle Paperwhite
- [ ] Kindle Oasis
- [ ] Kobo Clara
- [ ] Kobo Libra
- [ ] Nook GlowLight
- [ ] Boox
- [ ] reMarkable

### Night Reading

- [x] True Black AMOLED
- [x] Midnight Blue
- [ ] Deep Purple Night
- [ ] Forest Night
- [ ] Ocean Night
- [x] Warm Night
- [ ] Red Shift (reduces blue light)
- [ ] Amber Night
- [ ] Candlelight

### Eye Comfort

- [ ] Warm Sepia
- [ ] Cool Gray
- [ ] Soft Green
- [ ] Blue Light Filter
- [ ] f.lux Inspired (Warm)
- [ ] f.lux Inspired (Sunset)
- [ ] Night Shift

### Print/Book Inspired

- [ ] Vintage Paper
- [ ] Aged Parchment
- [ ] Library Classic
- [ ] Penguin Classics
- [ ] Oxford Press
- [ ] Cambridge Blue
- [ ] Manuscript

---

## Seasonal/Time-Based Themes

### Seasons

- [ ] Spring Blossom
- [ ] Summer Sun
- [ ] Autumn Leaves
- [ ] Winter Frost
- [ ] Cherry Blossom (Sakura)
- [ ] Maple Autumn
- [ ] Snow White

### Time of Day

- [ ] Morning Light
- [ ] Noon Bright
- [ ] Golden Hour
- [ ] Twilight
- [ ] Dusk
- [ ] Midnight

---

## Color-Specific Themes

### Blue Themes

- [ ] Ocean Blue
- [ ] Navy Dark
- [ ] Ice Blue
- [ ] Cobalt
- [ ] Royal Blue
- [ ] Sky Blue Light

### Green Themes

- [ ] Forest Green
- [ ] Mint Fresh
- [ ] Emerald
- [ ] Sage
- [ ] Matrix Green
- [ ] Terminal Green

### Purple Themes

- [ ] Amethyst
- [ ] Lavender
- [ ] Violet Dark
- [ ] Grape
- [ ] Royal Purple

### Red/Warm Themes

- [ ] Burgundy
- [ ] Rose
- [ ] Coral
- [ ] Terracotta
- [ ] Rust

### Brown/Earth Themes

- [ ] Coffee
- [ ] Chocolate
- [ ] Espresso
- [ ] Mocha
- [ ] Sandalwood
- [ ] Earth Tones

### Monochrome

- [ ] Grayscale Light
- [ ] Grayscale Dark
- [ ] Pure Black & White
- [ ] Ink
- [ ] Charcoal

---

## Brand/Platform Inspired

### Operating Systems

- [ ] macOS Monterey Light
- [ ] macOS Monterey Dark
- [ ] Windows 11 Light
- [ ] Windows 11 Dark
- [ ] Ubuntu Yaru Light
- [ ] Ubuntu Yaru Dark
- [ ] Elementary OS
- [ ] Pop!_OS

### Applications

- [ ] Discord Dark
- [ ] Slack Aubergine
- [ ] Notion Light
- [ ] Notion Dark
- [ ] Obsidian
- [ ] Bear Light
- [ ] Bear Dark
- [ ] iA Writer Light
- [ ] iA Writer Dark
- [ ] Ulysses Light
- [ ] Ulysses Dark
- [ ] Typora Light
- [ ] Typora Dark
- [ ] Zettlr

### Social/Web

- [ ] Twitter Dark
- [ ] Twitter Light
- [ ] Reddit Dark
- [ ] Reddit Light
- [ ] Hacker News
- [ ] Medium Light
- [ ] Medium Dark
- [ ] Dev.to Dark
- [ ] Hashnode Dark

---

## Regional/Cultural Themes

### Japanese

- [ ] Sakura
- [ ] Bamboo
- [ ] Zen Garden
- [ ] Wabi-Sabi
- [ ] Ukiyo-e

### Nordic

- [ ] Aurora Borealis
- [ ] Fjord
- [ ] Viking
- [ ] Nordic Ice

### Mediterranean

- [ ] Santorini
- [ ] Terracotta
- [ ] Olive Grove
- [ ] Azure Coast

---

## Special Purpose Themes

### Accessibility

- [ ] Colorblind Safe (Deuteranopia)
- [ ] Colorblind Safe (Protanopia)
- [ ] Colorblind Safe (Tritanopia)
- [ ] Large Text Optimized
- [ ] Reduced Motion

### Focus/Distraction-Free

- [ ] Minimal Dark
- [ ] Minimal Light
- [ ] Focus Mode Dark
- [ ] Focus Mode Light
- [ ] Typewriter
- [ ] Manuscript Edit

### Code Reading

- [ ] Syntax Dark
- [ ] Syntax Light
- [ ] Technical Documentation

---

## Custom Theme Support

- [x] Custom (User-defined colors via settings panel)
- [ ] Import from JSON
- [ ] Import from CSS
- [ ] Import from VS Code theme
- [ ] Theme sharing/export

---

## Statistics

**Total Themes Planned:** 250+
**Currently Implemented:** 48
**Progress:** ~19%

---

## Contributing

To add a new theme:

1. Add theme colors to `src/config/theme.rs`
2. Add CSS variables to `assets/style.css`
3. Update the theme selector in web UI
4. Mark the theme as implemented in this file

## Priority Queue

High priority themes to implement next:

1. ~~One Dark Pro~~ ✓
2. ~~GitHub Dark/Light~~ ✓
3. ~~Rosé Pine family~~ ✓
4. ~~Everforest family~~ ✓
5. ~~True Black AMOLED~~ ✓
6. ~~E-reader inspired themes (Kindle, Kobo)~~ ✓
7. ~~Accessibility themes (High Contrast)~~ ✓
8. ~~Material Dark~~ ✓
9. ~~Night Owl~~ ✓
10. ~~Atom One Dark/Light~~ ✓
11. ~~Palenight~~ ✓
12. ~~Ayu family~~ ✓
13. ~~Horizon~~ ✓
14. ~~Synthwave '84~~ ✓
15. Moonlight
16. Vitesse Dark/Light
17. More accessibility themes
