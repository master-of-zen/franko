...
impl SearchResults {
    ...
    pub fn new(
        ...
        let books_searched = results.iter()
            .map(|r| r.book_id.as_str())
            .collect::<std::collections::HashSet<_>>()
            .len();
        
        let chapters_searched = results.iter()
    ...
    pub fn by_book(&self) -> std::collections::HashMap<String, Vec<&SearchResult>> {
        ...
        for result in &self.results {
            grouped.entry(result.book_id.clone())
    ...
    pub fn by_chapter(&self) -> std::collections::HashMap<(String, usize), Vec<&SearchResult>> {
        let mut grouped = std::collections::HashMap::new();
        for result in &self.results {
            grouped.entry((result.book_id.clone(), result.chapter_index))
                .or_insert_with(Vec::new)
                .push(result);
        }
        grouped
...
impl TextSearcher {
    ...
    pub fn new(pattern: &str, case_sensitive: bool, regex_mode: bool) -> Result<Self, regex::Error> {
