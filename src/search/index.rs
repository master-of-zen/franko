...
use tantivy::schema::{Field, Schema, STORED, TEXT, STRING, FAST};
use tantivy::{doc, Index, IndexWriter, IndexReader, ReloadPolicy, TantivyDocument};
...
impl SearchIndex {
    ...
    pub fn index_book(&self, book: &Book) -> Result<()> {
        ...
        for (chapter_idx, chapter) in book.content.chapters.iter().enumerate() {
            for (block_idx, block) in chapter.blocks.iter().enumerate() {
                ...
                let chapter_title = chapter.title.clone().unwrap_or_else(|| format!("Chapter {}", chapter_idx + 1));
    ...
    pub fn search(&self, query: &str, limit: usize) -> Result<Vec<IndexEntry>> {
        ...
        let query_parser = QueryParser::for_index(&self.index, vec![self.content_field, self.chapter_title_field]);
        ...
        for (_score, doc_address) in top_docs {
            ...
            let book_id = doc.get_first(self.book_id_field)
                .and_then(|v| v.as_str())
                .unwrap_or_default()
                .to_string();
            
            let chapter_index = doc.get_first(self.chapter_index_field)
                .and_then(|v| v.as_u64())
                .unwrap_or(0) as usize;
            
            let block_index = doc.get_first(self.block_index_field)
                .and_then(|v| v.as_u64())
                .unwrap_or(0) as usize;
            ...
            let chapter_title = doc.get_first(self.chapter_title_field)
                .and_then(|v| v.as_str())
                .unwrap_or_default()
                .to_string();
    ...
    pub fn search_in_book(&self, book_id: &str, query: &str, limit: usize) -> Result<Vec<IndexEntry>> {
        self.reader.reload()
            .map_err(|e| FrankoError::Search(format!("Failed to reload: {}", e)))?;
        ...
        let query_parser = QueryParser::for_index(
            &self.index, 
            vec![self.book_id_field, self.content_field, self.chapter_title_field]
        );
        
        let parsed_query = query_parser.parse_query(&full_query)
            .map_err(|e| FrankoError::Search(format!("Failed to parse query: {}", e)))?;
...
mod tests {
    ...
    fn test_search_in_book() {
        ...
        let results = index.search_in_book("test-book-1", "Rust programming", 10).unwrap();
        assert!(!results.is_empty());
    }
